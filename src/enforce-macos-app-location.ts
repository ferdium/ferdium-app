...
export function enforceMacOSAppLocation() {
  ...
  const clickedButtonIndex = api.dialog.showMessageBoxSync({
    ...
    detail: 'Ferdi must live in the Applications folder to be able to run correctly.',
    buttons: [
    ...
    conflictHandler: conflict => {
      if (conflict === 'existsAndRunning') { // Can't replace the active version of the app
        api.dialog.showMessageBoxSync({
          type: 'error',
          message: 'Another version of Ferdi is currently running. Quit it, then launch this version of the app again.',
          buttons: [
            'OK',
          ],
        });
